...
function HeroCard({imgSrc, title1, title2, btn1, btn2}) {
  return (
    <div className="relative w-full h-[400px] md:h-[500px] lg:h-[600px] rounded-[30px] overflow-hidden mt-10 mx-auto flex items-end justify-start bg-[url('/herocard.jpg')] bg-cover bg-center p-8 md:p-14">
        ...
        <div className="flex flex-col justify max-w-full lg:max-w-[50%]">
          ...
          <div className="flex flex-row justify-center lg:justify-start items-center mt-4">
            <Link to={btn1.link} className="rounded-[30px] font-archivo bg-white text-[#454545] transform duration-300 hover:bg-[#f0f0f0] p-5 font-semibold text-[28px]">
            {btn1.text}
          </Link>
          <Link to={btn2.link} className="rounded-[30px] font-archivo bg-[#FFFFFF4D] text-white transform duration-300 hover:bg-[#ffffff67] ms-4 p-5 font-semibold text-[28px]">
            {btn2.text}
          </Link>
          </div>
        </div>
